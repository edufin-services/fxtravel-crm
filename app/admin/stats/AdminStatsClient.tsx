...
import { Stage, STAGES, CHANNELS } from "@/lib/constants";
import { Lead, Task, Contact, User } from "@/lib/db";
...
function getLeadBranchKey(l: Lead): "delhi" | "kolkata" | "unassigned" {
  ...
  if (c.includes("delhi") || bName.includes("delhi") || bId.includes("delhi")) return "delhi";
  if (c.includes("kolkata") || bName.includes("kolkata") || bId.includes("kolkata")) return "kolkata";
  return "unassigned";
}
...
type BranchFilter = "all" | "delhi" | "kolkata" | "unassigned";
type SortField = "leads" | "confirmed" | "winRate" | "value";
...
export default function AdminStatsClient({
  ...
  const [agentSort, setAgentSort] = useState<SortField>("confirmed");

  // User Map for fast name lookup
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((l) => {
      ...
      return new Date(l.createdAt).getTime() >= threshold;
    });
  }, [leads, branchFilter, timeframe]);
  ...
  // ── Branch Breakdown Metrics ──────────────────────────────────────────────
  const branchMetrics = useMemo(() => {
    const delhiLeads = filteredLeads.filter((l) => getLeadBranchKey(l) === "delhi");
    const kolkataLeads = filteredLeads.filter((l) => getLeadBranchKey(l) === "kolkata");
    const unassignedLeads = filteredLeads.filter((l) => getLeadBranchKey(l) === "unassigned");

    const summarize = (list: Lead[], name: string, region: string) => {
      const count = list.length;
      const confirmed = list.filter((l) => l.stage === "Confirmed").length;
      const closed = list.filter((l) => l.stage === "Closed").length;
      const value = list.reduce((s, l) => s + (l.value || 0), 0);
      const win = count > 0 ? Math.round((confirmed / count) * 100) : 0;
      return { name, region, count, confirmed, closed, value, win };
    };

    return [
      summarize(delhiLeads, "Delhi NCR Hub", "North Region"),
      summarize(kolkataLeads, "Kolkata Hub", "East Region"),
      summarize(unassignedLeads, "Unassigned Routing", "Direct / Pending"),
    ];
  }, [filteredLeads]);
  ...
  const agentLeaderboard = useMemo(() => {
    ...
    for (const l of filteredLeads) {
      if (!statsByUser[l.ownerId]) {
        statsByUser[l.ownerId] = {
          user: { id: l.ownerId, name: "Unassigned / Direct", email: "", company: "", passwordHash: "", createdAt: "" },
          total: 0,
          initial: 0,
          connected: 0,
          confirmed: 0,
      ...
      const entry = statsByUser[l.ownerId];
      entry.total++;
      entry.value += l.value || 0;
      if (l.stage === "Initial") entry.initial++;
      if (l.stage === "Connected") entry.connected++;
      if (l.stage === "Confirmed") {
        entry.confirmed++;
        entry.confirmedValue += l.value || 0;
      }
      if (l.stage === "Closed") entry.closed++;
    }

    let list = Object.values(statsByUser).map((item) => {
      const winPct = item.total > 0 ? (item.confirmed / item.total) * 100 : 0;
      return { ...item, winPct };
    });

    if (searchAgent.trim()) {
      const q = searchAgent.toLowerCase().trim();
      list = list.filter((a) => a.user.name.toLowerCase().includes(q) || a.user.email.toLowerCase().includes(q));
    }

    list.sort((a, b) => {
      if (agentSort === "confirmed") return b.confirmed - a.confirmed || b.total - a.total;
      if (agentSort === "winRate") return b.winPct - a.winPct || b.confirmed - a.confirmed;
      if (agentSort === "value") return b.value - a.value;
      return b.total - a.total;
    });

    return list;
  }, [filteredLeads, users, searchAgent, agentSort]);
  ...
  return (
    <div className="space-y-7 pb-12 font-sans text-zinc-900">
      {/* ── Executive Clean White Header ──────────────────────────────────── */}
      <div className="rounded-3xl border border-zinc-200/90 bg-white p-6 sm:p-7 shadow-2xs">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2.5">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-purple-50 text-purple-700 font-bold border border-purple-200/60 shadow-2xs">
            ...
            <p className="text-xs text-zinc-500 sm:text-sm font-medium max-w-2xl">
              Global conversion rates, branch revenue, channel quality, and executive sales performance across <strong className="text-zinc-800">{users.length} agents</strong>.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <Link
              href="/admin/leads"
              className="inline-flex items-center gap-2 rounded-2xl bg-zinc-50 hover:bg-zinc-100 border border-zinc-200/80 px-4 py-2.5 text-xs font-bold text-zinc-700 hover:text-zinc-900 transition-colors shadow-2xs cursor-pointer"
            >
              <span>Global Board View</span>
            ...
            <Link
              href="/admin/branches"
              className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 px-4 py-2.5 text-xs font-extrabold text-white transition-all shadow-md shadow-emerald-600/20 active:scale-95 cursor-pointer"
            >
        ...
        <div className="mt-5 pt-4 border-t border-zinc-100 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-extrabold text-zinc-400 uppercase tracking-wider">Time:</span>
            ...
            {/* Branch Filter */}
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-extrabold text-zinc-400 uppercase tracking-wider">Branch:</span>
              <div className="flex items-center gap-1 bg-zinc-100/90 p-1 rounded-xl border border-zinc-200/70">
                {[
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <div className="group relative overflow-hidden rounded-2xl border border-zinc-200/90 bg-white p-5 shadow-xs hover:border-blue-300 hover:shadow-md transition-all duration-200">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold uppercase tracking-wider text-zinc-400">Total System Leads</span>
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 flex items-center gap-1.5 text-xs font-bold text-blue-600">
              <span>+{newThisWeek} intake this week</span>
            </div>
          </div>
          <div className="mt-3 pt-3 border-t border-zinc-100 text-[11px] text-zinc-400 font-medium truncate">
            Managed across {users.length} sales agents
          </div>
        </div>

        {/* Card 2: Confirmed Won (Strictly Confirmed) */}
        <div className="group relative overflow-hidden rounded-2xl border border-emerald-200/90 bg-gradient-to-br from-emerald-50/50 via-white to-teal-50/30 p-5 shadow-xs hover:border-emerald-300 hover:shadow-md transition-all duration-200">
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 flex items-center gap-1 text-xs font-bold text-emerald-800">
              <span>{winRate.toFixed(1)}% Org Win Rate</span>
            </div>
          </div>
        ...
        {/* Card 3: Closed / Lost Deals (Strictly Closed) */}
        ...
        <div className="group relative overflow-hidden rounded-2xl border border-zinc-200/90 bg-white p-5 shadow-xs hover:border-zinc-300 hover:shadow-md transition-all duration-200">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold uppercase tracking-wider text-zinc-400">Team Execution</span>
      ...
      {/* ── Visual Conversion Funnel & Drop-off Flow ───────────────────────── */}
      <div className="rounded-3xl border border-zinc-200/90 bg-white p-6 sm:p-8 shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-zinc-100 pb-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="flex h-7 w-7 items-center justify-center rounded-xl bg-gradient-to-br from-purple-600 to-indigo-600 text-white shadow-xs text-xs font-black">
                ▼
              </span>
              <h2 className="text-lg font-black text-zinc-900 tracking-tight">
                Global Pipeline Conversion Funnel &amp; Leakage
              </h2>
            </div>
            <p className="text-xs text-zinc-400 mt-1 font-medium">
              Lead progression flow from intake through discussion to finalized booking vs. drop-offs
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {/* Stage 1 */}
          <div className="rounded-2xl border border-blue-200/80 bg-gradient-to-b from-blue-50/50 to-white p-5 space-y-3">
            <div className="flex items-center justify-between">
          ...
          {/* Stage 2 */}
          <div className="rounded-2xl border border-amber-200/80 bg-gradient-to-b from-amber-50/50 to-white p-5 space-y-3">
            <div className="flex items-center justify-between">
            ...
            <div>
              <p className="text-xs font-bold text-zinc-500">Connected Prospects</p>
          ...
          {/* Stage 3 */}
          <div className="rounded-2xl border-2 border-emerald-300 bg-gradient-to-b from-emerald-50/60 via-emerald-50/20 to-white p-5 space-y-3 shadow-xs">
            <div className="flex items-center justify-between">
          ...
          {/* Leakage */}
          <div className="rounded-2xl border border-rose-200/80 bg-gradient-to-b from-rose-50/40 to-white p-5 space-y-3">
            <div className="flex items-center justify-between">
            ...
            <div>
              <p className="text-xs font-bold text-zinc-500">Closed / Dropped Deals</p>
        ...
        <div className="space-y-2 pt-2">
          <div className="flex justify-between text-xs font-bold text-zinc-600">
            ...
            <span>{totalLeads} Total Organization Leads</span>
          </div>
      ...
      <div className="rounded-3xl border border-zinc-200/90 bg-white p-6 sm:p-7 shadow-xs space-y-5">
        <div className="flex items-center justify-between border-b border-zinc-100 pb-4">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Pipeline distribution and confirmed deals across regional operations</p>
          </div>
          <Link
            href="/admin/branches"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {branchMetrics.map((b) => (
            <div
              key={b.name}
              className="rounded-2xl p-5 border border-zinc-200/90 bg-zinc-50/50 hover:bg-white hover:border-zinc-300 hover:shadow-xs transition-all space-y-4"
            >
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="h-2.5 w-2.5 rounded-full bg-blue-600" />
                    <h3 className="font-black text-sm text-zinc-900">{b.name}</h3>
                  </div>
                  <p className="text-[11px] font-bold text-zinc-400 mt-0.5">{b.region}</p>
                </div>
                <span className="px-2.5 py-1 rounded-xl text-xs font-black bg-blue-50 text-blue-700 border border-blue-200">
                  {b.count} Leads
                </span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-center pt-1">
                <div className="p-2 rounded-xl bg-white border border-zinc-100">
                  <div className="text-xs font-black text-zinc-800">{b.count}</div>
                  <div className="text-[10px] text-zinc-400 font-bold uppercase">Total</div>
                </div>
                <div className="p-2 rounded-xl bg-emerald-50/70 border border-emerald-100">
                  <div className="text-xs font-black text-emerald-700">{b.confirmed}</div>
                  <div className="text-[10px] text-emerald-800 font-bold uppercase">Won</div>
                </div>
                <div className="p-2 rounded-xl bg-rose-50/70 border border-rose-100">
                  <div className="text-xs font-black text-rose-700">{b.closed}</div>
                  <div className="text-[10px] text-rose-800 font-bold uppercase">Lost</div>
                </div>
              </div>

              <div className="pt-2 border-t border-zinc-200/60 flex items-center justify-between text-xs font-bold">
                <span className="text-zinc-500">Pipeline Value</span>
                <span className="text-purple-700 font-black">{formatINR(b.value)}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-3xl border border-zinc-200/90 bg-white p-6 sm:p-7 shadow-xs space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-zinc-100 pb-4">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Individual conversion rates, won bookings, and pipeline value</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <input
              ...
              className="text-xs rounded-xl border border-zinc-200 px-3 py-2 bg-zinc-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 w-48 sm:w-60"
            />

            {/* Sort Toggle */}
            <div className="inline-flex p-0.5 rounded-xl bg-zinc-100 border border-zinc-200/80">
              {[
                { id: "confirmed", label: "Most Won" },
                ...
                <button
                  key={s.id}
                  onClick={() => setAgentSort(s.id as SortField)}
                  className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    agentSort === s.id ? "bg-white text-zinc-900 shadow-2xs" : "text-zinc-500 hover:text-zinc-800"
                  }`}
                >
                  {s.label}
                </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-zinc-200/80 text-[11px] font-extrabold uppercase tracking-wider text-zinc-400">
                <th className="pb-3 pl-2">Agent</th>
                <th className="pb-3 text-center">Assigned Leads</th>
                <th className="pb-3 text-center">In Pipeline</th>
                <th className="pb-3 text-center">Confirmed Won</th>
                <th className="pb-3 text-center">Closed Lost</th>
                <th className="pb-3 text-center">Win Rate</th>
                <th className="pb-3 text-right">Pipeline Value</th>
                <th className="pb-3 text-right pr-2">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100">
              {agentLeaderboard.map((item, idx) => (
                <tr key={item.user.id || idx} className="hover:bg-zinc-50/70 transition-colors group">
                  <td className="py-3 pl-2">
                    <div className="flex items-center gap-3">
                      <div className={`flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br ${getAvatarGrad(item.user.name)} text-white font-black text-xs shadow-2xs`}>
                        {getInitials(item.user.name)}
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-zinc-900 truncate">{item.user.name}</p>
                        <p className="text-[11px] text-zinc-400 truncate">{item.user.email || item.user.company || "Agent"}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 text-center font-black text-zinc-800">{item.total}</td>
                  <td className="py-3 text-center font-bold text-amber-700">{item.initial + item.connected}</td>
                  <td className="py-3 text-center">
                    <span className="inline-flex items-center px-2 py-0.5 rounded-md font-black bg-emerald-50 text-emerald-700 border border-emerald-200">
                      {item.confirmed}
                    </span>
                  </td>
                  <td className="py-3 text-center">
                    <span className="inline-flex items-center px-2 py-0.5 rounded-md font-bold bg-rose-50 text-rose-700 border border-rose-200">
                      {item.closed}
                    </span>
                  </td>
                  <td className="py-3 text-center">
                    <span className={`font-black text-xs ${item.winPct > 0 ? "text-emerald-700" : "text-zinc-400"}`}>
                      {item.winPct.toFixed(1)}%
                    </span>
                  </td>
                  <td className="py-3 text-right font-black text-purple-800">
                    {formatINR(item.value)}
                  </td>
                  <td className="py-3 text-right pr-2">
                    {item.user.id ? (
                      <Link
                        href={`/admin/agents/${item.user.id}`}
                        className="inline-flex items-center gap-1 text-xs font-bold text-purple-700 hover:text-purple-900 bg-purple-50 hover:bg-purple-100 border border-purple-200/80 px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
                      >
                        Board →
                      </Link>
                    ) : (
      ...
      {/* ── Mid Section: Monthly Trends + Channel Quality ────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-7">
        {/* Left (7 Cols): 6-Month Timeline Chart */}
        <div className="lg:col-span-7 rounded-3xl border border-zinc-200/90 bg-white p-6 sm:p-7 shadow-xs space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-zinc-100 pb-4">
            <div>
              <h2 className="text-base font-black text-zinc-900 tracking-tight flex items-center gap-2">
                ...
                6-Month Organization Trends
              </h2>
              <p className="text-xs text-zinc-400 mt-0.5">Historical lead volume and value progression across company</p>
            </div>
                  ...
                  <div key={bar.key} className="flex flex-1 flex-col items-center gap-2 h-full justify-end group">
                    ...
                    <div className="flex h-36 w-full items-end justify-center">
                      <div
                        ...
                        style={{
                          height: `${fillPct}%`,
                          background: metricVal > 0 ? "linear-gradient(to top, #4f46e5, #818cf8)" : "#f4f4f5",
                        }}
                      >
                        {bar.confirmedCount > 0 && chartMetric === "count" && (
                          <span
                            className="absolute -top-2 left-1/2 -translate-x-1/2 flex h-4 w-4 items-center justify-center rounded-full bg-emerald-600 text-white text-[9px] font-bold ring-2 ring-white"
          ...
          <div className="flex items-center justify-between pt-3 border-t border-zinc-100 text-xs text-zinc-400 font-medium">
            <span>Total CRM Database: {leads.length} leads</span>
            <span className="flex items-center gap-1.5 text-indigo-700 font-bold">
              <span className="h-2 w-2 rounded-full bg-indigo-500" />
              Realized Intake Trajectory
            </span>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {channelStats.map((ch) => {
              ...
              return (
                <div
                  key={ch.name}
                  className="rounded-2xl p-3 bg-zinc-50/70 border border-zinc-200/70 hover:bg-white hover:border-zinc-300 hover:shadow-2xs transition-all space-y-2"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
