...
export async function POST(request: NextRequest, ctx: Ctx) {
  ...
  const safeName = `3rd-payment-invoice-${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
  ...
  lead.thirdPaymentInvoices = [...(lead.thirdPaymentInvoices ?? []), invoice];
  await lead.save();
...
export async function DELETE(request: NextRequest, ctx: Ctx) {
  ...
  lead.thirdPaymentInvoices = (lead.thirdPaymentInvoices ?? []).filter((d: { url: string }) => d.url !== url);
  await lead.save();
